    ...
    if(customerId){    
        async function getCustomerBookmarks() {
              ...
              try {
                ...
                let sql = `BEGIN
                            corp0710.get_customer_companies(:p_web_customer_id,:p_record_set,:ERR_CD,:ERR_MSG) ;
                        END ;`;

                let bindVars = {
                    p_web_customer_id: customerId,
                ...
                result = await connection.execute(sql, bindVars);
                 const cursor = result.outBinds.p_record_set;
                    ...
                    if(companiesArray.length > 0){
                        res.status(402).send({message: 'SUCCESS',data:companiesArray});
                    }
                    else{
                        res.status(401).send({message: 'FAILURE',data:'No companies for the customer'});
                    }
