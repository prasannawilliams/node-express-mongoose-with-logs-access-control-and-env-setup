...
exports.get = async (requestData, res) => {
    ...
    if(customerId){    
      try {
        let customer_result = await usermodel.customer_records('get_customer_emails','p_customer_id',customerId);
        if (customer_result.length > 0) {
          res.status(200).send({ message: 'SUCCESS', data: customer_result });
      }
      else {
          res.status(404).send({ message: 'FAILURE', data: lang.NO_EMAILLIST_FOUND });
      }
      }
      catch(err) {
